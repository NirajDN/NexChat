        ...
        <div className='px-5 text-xs'>
            ...
            gap-4 opacity-80'>
                {msgImages.map((url,index)=>(
                    ...
                    className='curson-pointer rounded'>
                        <img src={url} alt="" className='h-half rounded-md'/>
                    </div>
                ))}
            </div>
        </div>
        <button onClick={()=> logout()}
         className='absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-linear-to-r from-purple-400 
