...
const ChatContainer = () => {
    ...
    useEffect(()=>{
        if(scrollEnd.current && messages){
            scrollEnd.current.scrollIntoView({behvaior:"smooth"})
        }
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
            ...
            <img src={assets.help_icon} alt="" className='max md:hidded max-w-5'/>
        </div>
        {/*-----Chat area-----*/}
        ...
        <div className='absolute bottom-0 left-0 flex items-center gap-3 p-3'>
            <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                ...
                onKeyDown={(e)=> e.key === "Enter" ? handleSendMessage(e): null}
                 type="text" placeholder="Send a message" className="flex-1 w-[450px] text-sm p-3 
                border-none rounded-lg outline-none text-white placeholder-gray-400"/>
                <input onChange={handleSendImage}
                 type="file" id="image" accept="image/png, image/jpeg, image/gif, video/mp4, video/webm, video/ogg" hidden/>
                <label htmlFor="image">
